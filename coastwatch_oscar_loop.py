...
start = datetime.datetime(syyyy,smm,sdd)
end = datetime.datetime(eyyyy,emm,edd)

tinx=0
while start <= end:
    yyyy=int(datetime.datetime.strftime(start,'%Y'))
    mm=int(datetime.datetime.strftime(start,'%m'))
    dd=int(datetime.datetime.strftime(start,'%d'))
    ...
    plt.savefig('coastwatch_' + str(tinx).zfill(2),dpi=50)
